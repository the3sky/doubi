...
Service_brook(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/the3sky/doubi/master/service/brook_centos" -O /etc/init.d/brook; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/the3sky/doubi/master/service/brook_debian" -O /etc/init.d/brook; then
			echo -e "${Error} Brook服务 管理脚本下载失败 !" && rm -rf "${file}" && exit 1
		fi
		chmod +x "/etc/init.d/brook"
